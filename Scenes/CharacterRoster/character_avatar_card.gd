...
# 不像 PartyEdit 的 HeroCard 那樣還帶名字/battle_cost 形狀——名字
# 改用 tooltip 顯示。點擊後 hero_selected 訊號通知上層換顯示的
...
func _ready() -> void:
	...
	expand_icon = true
	if hero != null:
		tooltip_text = hero.full_name
		if not hero.face_path.is_empty():
			icon = load(hero.face_path) as Texture2D
	_apply_style()
	pressed.connect(func(): hero_selected.emit(hero))
