...
func _ready() -> void:
	...
	# 寄信國家在下面),取代原本各自獨立一顆固定尺寸羊皮紙面板的設計。
...
## 上方「寄信國家」區塊:下方畫一條線跟「聯姻角色」區塊分隔,六國按鈕排成一列 grid,附
## 好感度等級——不含候選人清單,候選人改在確認之後的盲選 Dialogue 事件裡才生成(見
...
## 下方「聯姻角色」區塊:最後一塊、後面沒有區塊接著,不需要分隔線,但仍要蓋掉引擎預設的
## PanelContainer 深色底,裡面是已篩過未婚角色的 CharacterSelectBar,卡片網格多長就自然
## 往下長多高(只有最外層 ActionPanel 負責捲動,見 action_panel.gd 開頭註解)。
