...
## 移動/尋路的共用計算服務,從 BattleCharacter 搬出來——BattleCharacter 只保留「套用移動結果、
...
static func move_steps(actor: BattleCharacter) -> int:
...
## 回傳完整路徑(可能為了繞路而轉彎),呼叫端(BattleCharacter.move()/move_away())負責
## 實際套用 grid_pos 與記錄 MoveEvent。
static func plan_path(actor: BattleCharacter, target: BattleCharacter, away: bool, atk_range: int) -> Array[Vector2i]:
...
## 回傳路徑,空陣列代表不需要移動。
static func plan_kite_path(actor: BattleCharacter, target: BattleCharacter, atk_range: int) -> Array[Vector2i]:
	if atk_range <= 1:
		return []
...
## 卡死、停止移動)。
static func _next_step(actor: BattleCharacter, from: Vector2i, target: BattleCharacter, away: bool) -> Vector2i:
	var dx := 0
	var dy := 0
...
static func _is_blocked_by_enemy(actor: BattleCharacter, pos: Vector2i) -> bool:
