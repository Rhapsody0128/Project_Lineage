...
# =========================================================
# 疊加在目前畫面最上層的獨立選人彈窗——跟 Scenes/ActionPanel/action_panel.gd 是不同的
# CanvasLayer(layer 設得比它高,見 LAYER),疊上來的當下底下的 ActionPanel(根據地建築
# 面板)完全不受影響,繼續留在畫面上(半透明黑幕之後)、也不會被釋放——挑一位角色只是
# 「疊一層蓋滿畫面的視窗蓋在上面」,不是「把底下的面板整包替換/釋放掉再重建」。呼叫端
...
#
# 外殼(木框羊皮紙面板 + 標題列 + × 關閉鈕)比照 ActionPanel 的既有視覺語言,但不使用
# ActionPanel 本身——近全螢幕(四邊留 Scripts/UI/fullscreen_overlay.gd 的
# FullscreenOverlay.SCREEN_MARGIN,跟 Scenes/Marriage/ 底下的告白/求婚面板共用同一個
# margin,長相統一),不切場景。
# =========================================================
...
## 所以取更高的 30(跟 FullscreenOverlay.LAYER 一致)。
const LAYER := 30
## 跟 Scripts/UI/fullscreen_overlay.gd 的 FullscreenOverlay.SCREEN_MARGIN 同一個數字——
## 兩者都是「近全螢幕、四邊留白」的外殼,margin 要一致,不要各自調整。
...
func open(title: String, characters: Array[Character], card_factory: Callable, initial_sort_key: int, on_confirmed: Callable, confirm_label: String = CharacterSelectPanel.DEFAULT_CONFIRM_LABEL, initial_focus: Character = null) -> void:
